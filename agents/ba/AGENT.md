...
작업에 필요한 정보(마스터 기획서, 배경 레퍼런스, 내부 기획서 등)를 파악한다.

### Step 2: 지역/배경 분석

컨텍스트의 마스터 기획서에서 주요 지역 및 배경 환경 아트 관련 챕터를 파악하여 분석한다.
...
컨텍스트의 중세 건축 레퍼런스 참조: 4스테이지 배경 시대적 근거 확보:
...
컨텍스트의 판타지 환경 가이드 참조: 2D 환경 설계 원칙/색상 팔레트 적용:
- 각 스테이지별 환경 무드 팔레트
- 자연/인공 요소 배합 비율
- 레이어별 디테일 수준 기준
...
컨텍스트의 Unity 2D Lighting(URP) 가이드 참조: 스테이지별 조명 연출 기준 설정:
...
`{tool:image_generate}`로 배경 컨셉 참고 이미지 생성:
- 스테이지별 무드보드 (색상+구조물+자연)
- 조명 before/after 비교 샘플

### Step 8: Unity Tilemap & Parallax 기준 확인

컨텍스트의 Unity Tilemap & Parallax 가이드 참조: 원경/중경/근경 구현 기준 확인:
