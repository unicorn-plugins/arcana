...
작업에 필요한 정보(마스터 기획서, Unity/기술 레퍼런스, 내부 기획서 등)를 파악한다.

### Step 2: 기술 기획 분석
컨텍스트의 마스터 기획서에서 기술 기획 관련 챕터를 파악하여 분석한다.
게임 시스템 요구사항 및 애니메이션/VFX 연출 관련 챕터도 파악한다.
...
### Step 9: 데이터 구조 설계
`{tool:art_pipeline}` 및 기획서 데이터 구조 관련 챕터 기반 외부 데이터 시트 포맷 정의:
