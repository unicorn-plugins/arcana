...
description: 외부호출 스킬(ext-{대상플러그인}) 제거
type: external
user-invocable: true
---
...
사용자가 `/arcana:remove-ext-skill`로 호출하여 기존 외부호출 스킬(ext-{대상플러그인})을 제거할 수 있게 함.

## 활성화 조건

사용자가 `/arcana:remove-ext-skill` 호출 시 또는 "외부 스킬 제거", "ext-skill 제거" 키워드 감지 시.
...
`skills/help/SKILL.md`의 명령 테이블에서 `/arcana:ext-{대상플러그인}` 행 제거.
제거 완료 메시지 출력.
